# Notices and Attribution

## Standards Reference

This curriculum is designed with reference to published computer science education frameworks. It is an independent work and carries no endorsement from the organizations referenced below.

### CSTA PK–12 Computer Science Standards

CSTA published the **2026 CSTA PK–12 Computer Science Standards** in summer 2026. The revised standards use a new grade-band structure and new identifiers, replacing the Level 1A/1B/2/3A/3B structure used in the **CSTA K–12 Computer Science Standards, Revised 2017**.

The current **Python Foundations — Grades 9–12** course was originally developed with references to the 2017 standards. Any public `3A-...` or other 2017 identifiers should therefore be read as **legacy design references**, not as claims of current alignment to the 2026 standards.

A lesson-by-lesson remap to the 2026 standards is part of the curriculum's ongoing quality-assurance work. New or revised public standards references will use the 2026 framework once that remap has been verified.

The 2026 CSTA PK–12 Computer Science Standards are published by the Computer Science Teachers Association and are licensed under the **Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International License (CC BY-NC-SA 4.0)**. CSTA provides a suggested citation for the 2026 standards on its official standards site.

CSTA's Curriculum Alignment Review provides independent validation of curriculum alignment to the 2026 standards. **This curriculum has not received a CSTA Curriculum Alignment Review or Seal of Alignment.** References to CSTA standards identify concepts used during curriculum design and do not represent CSTA certification, approval, endorsement, or validated alignment.

Unless specifically stated otherwise, public portfolio samples use standards names or identifiers rather than republishing substantial standards text.

### AP® Computer Science Principles

References to AP® Computer Science Principles Big Ideas, learning objectives, and related framework concepts are based on the publicly available College Board course framework and are provided to help educators identify related instructional content.

**AP® and Advanced Placement® are registered trademarks of the College Board.**

The College Board is not affiliated with and does not endorse this project.

This curriculum has not been produced, reviewed, approved, or endorsed by the College Board.

This curriculum does not cover the complete AP® Computer Science Principles framework and should not be represented as an authorized AP® course.

Schools wishing to offer a course using the AP® designation are responsible for completing the College Board AP Course Audit process independently.

### Verification

Educational standards and course frameworks may be revised.

Educators, schools, districts, and other users should confirm standards identifiers and framework requirements against the current official publications before using the materials for curriculum adoption, standards reporting, course authorization, or district approval.

---

## Code Verification

Every Python example and interpreter message **published in this repository** is verified through execution using **Python 3.12** before public release.

Python interpreter messages can change between versions. Differences between the output shown in these materials and another Python environment are therefore possible and can provide useful opportunities for classroom discussion.

---

## Trademarks

Python is a registered trademark in the United States of the Python Software Foundation.

Replit, Google Colab, Trinket, and other third-party product or platform names referenced in these materials are trademarks or property of their respective owners.

References to these products and platforms are descriptive and instructional and do not imply affiliation, sponsorship, or endorsement.

---

## Copyright

© 2026 Marsharine A. Simpson. All rights reserved except for the limited permissions expressly granted in [LICENSE](LICENSE).

Third-party materials remain subject to their respective copyrights, licenses, and trademark rights.
